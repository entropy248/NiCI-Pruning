...
def main(args):
    ...
    if os.path.isdir(args.model_path):
        ...
        if args.teacher_path is not None:
            print("Loading teahcer model from {}".format(args.teacher_path))
            teacher_pipeline = DDPMPipeline.from_pretrained(args.teacher_path)
            teacher_model = teacher_pipeline.unet.eval()
    ...
    if accelerator.is_main_process:
        if 'cifar' in args.model_path:
            example_inputs = {'sample': torch.randn(1, 3, 32, 32).to(accelerator.device),
                              'timestep': torch.ones((1,)).long().to(accelerator.device)}
        else:
            example_inputs = {'sample': torch.randn(1, 3, 256, 256).to(accelerator.device),
                              'timestep': torch.ones((1,)).long().to(accelerator.device)}
    ...
    for epoch in range(first_epoch, num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            model.train()
            # Skip steps until we reach the resumed # (step and step < resumed_step)
            ...
            with accelerator.accumulate(model):
                ...
                # out = model(clean_images, timesteps)
                # model_output, student_attn = out.sample, out.attn_tuple
                # print(student_attn[0][0])
                # assert 0
                b, c, w, h = model_output.shape

                if args.teacher_path is not None:
                    with torch.no_grad():
                        teacher_output = teacher_model(noisy_images, timesteps).sample
                        # t_out = teacher_model(clean_images, timesteps)
                        # teacher_output, teacher_attn = t_out.sample, t_out.attn_tuple

                    distillation_loss = F.kl_div(
                        F.log_softmax(model_output.reshape((b*c, w*h)) / args.tau, dim=-1),
                        F.log_softmax(teacher_output.reshape((b*c, w*h)) / args.tau, dim=-1),
                        reduction='sum',
                        log_target=True
                    ) * (args.tau * args.tau)

                    ori_loss = (noise - model_output).square().sum(dim=(1, 2, 3)).mean(dim=0)
                    # ori_loss = torch.nn.functional.mse_loss(model_output, clean_images)


                    loss = (ori_loss + args.alpha * distillation_loss)
                    # loss = (ori_loss + args.beta * attn_losses)

                    logs = {"loss": loss.detach().item(), "kd_loss": args.alpha * distillation_loss.detach().item(),
                            "ori_loss": ori_loss.detach().item(),
